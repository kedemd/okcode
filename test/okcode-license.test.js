...
// _C constants). @kedem/okdb is a symlink to okdb-src, so this is the same
// module instance okdb verifies with.
...
const { _C } = require('@kedem/okdb/lib/okdb-license');
const codec = require('@kedem/okdb/lib/okdb-license-codec');
...
const FP = createHash('sha256')
    ...
    .slice(0, 8)
    .toString('hex');
_C.PUBLIC_KEY_PEM = PUB;
_C.PUB_FINGERPRINT = FP;
...
describe('okcode.open({ license })', () => {
    ...
    it('installs the license into its own store; effective at once; a reopen is idempotent', async () => {
    ...
    it('a node-bound license that needs activation is reported, not fatal; { blob, activation } activates', async () => {
        ...
        oc = await okcode.open({ path: store('std'), license: { blob, activation: activationFor(rawBuf, nodeId) } });
        try {
            assert.equal((await oc.status()).license.status, 'active');
    ...
    it('an invalid license refuses to open (LICENSE_INVALID) and leaves the store closed', async () => {
        ...
        const forged = codec.encodeLicenseBlob(payload, cryptoSign(null, payload, other));
    ...
    it('with a host db, `license` is ignored (the host owns licensing)', async () => {
        ...
        await db.open();
        const { lines, log } = collect();
        const oc = await okcode.open({ db, license: makeLicense().blob, log });
        try {
...
describe('cli --license', () => {
    ...
    before(() => {
        ...
        fs.writeFileSync(
            preload,
            `const { _C } = require(${JSON.stringify(require.resolve('@kedem/okdb/lib/okdb-license'))});\n` +
